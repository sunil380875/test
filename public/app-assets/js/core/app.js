    ...
    if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
      ...
      if (value != '') {
        ...
        var $startList = '',
          ...
          $bookmarkIcon = '',
          $defaultList = '',
          $searchListHeader = '<li class="d-flex align-items-center">' +
          '<a href="#">' +
          '<h6 class="section-label mt-75 mb-0 apicontents">Contents</h6>' +
          '</a>' +
          '</li>',
          $searchListAPI = '',
          a = 0;
        ...
        if (value.length>1) {
          $.post(window.location.protocol + '//' + window.location.host + "/search", { search: value })
          .fail(function() {
          ...
          .done(function(datas) {
            ...
            if (datas.data && datas.data.length) {
              for (var i = 0; i < datas.data.length; i++) {
                ...
                // if (
                //   // !(datas.listItems[i].name.toLowerCase().indexOf(value) == 0) &&
                //   // datas.listItems[i].name.toLowerCase().indexOf(value) > -1 &&
                //   a < 5
                // ) {
                  let full = datas.data[i];
                  ...
                  '">' +
                  '<a class="d-flex align-items-center justify-content-between w-100" href="' +
                  window.location.protocol + '//' + window.location.host + (full["user_role"]["role"]=='user'?"/user":"/agent") + '/view/' + full["_id"] +
                  '">' +
                  '<div class="d-flex justify-content-start align-items-center">' +
                  ...
                  '</div>' +
                  '<span>' +
                  '<span class="user_name text-truncate text-body fw-bolder">' +
                  $name +
                  '</span><br>' +
                  ...
                  // '<li class="d-flex justify-content-left align-items-center auto-suggestion">' +
                  // '<div class="avatar-wrapper">' +
                  // '<div class="avatar ' +
                  // colorClass +
                  // ' me-1" style="cursor: default !important;">' +
                  ...
                  // '</div>' +
                  // '</li>';
                  $searchListAPI += $row_output;
                  
                  // a++;
                // }
              }
              if ($searchListAPI) {
                $htmlList = $htmlList.concat($searchListHeader.concat($searchListAPI));
              }
        ...
        $htmlList = '';
          ...
          if ($otherList) {
            $htmlList = $htmlList.concat($pageList.concat($otherList));
          }
          $('ul.search-list').html($htmlList);
          ...
          // if ($otherList) {
          //   $htmlList = $pageList.concat($otherList);
          // } 
          // else {
          //   $htmlList = $pageList.concat(`<li class="auto-suggestion current_item"><a class="d-flex align-items-center justify-content-between w-100"><div class="d-flex justify-content-start align-items-center"><span>Nothing Found</span></div></a></li>`);
          // }
          // $('ul.search-list').html($htmlList);
        ...
        // console.log($searchListAPI, '$searchListAPI')
        // console.log($otherList, '$otherList')
        // if ($searchListAPI.length && $otherList.length) {
        //   $htmlList = $searchListHeader.concat($searchListAPI);
        //   $htmlList = $htmlList.concat($pageList.concat($otherList));
        // } else
        // if ($searchListAPI.length) {
        //   $htmlList = $searchListHeader.concat($searchListAPI);
        // } else
        // if ($otherList.length) {
        //   console.log($otherList, '$otherList')
        ...
        // } 
        // else {
        //   $htmlList = `<li class="auto-suggestion current_item"><a class="d-flex align-items-center justify-content-between w-100"><div class="d-flex justify-content-start align-items-center"><span>Nothing Found</span></div></a></li>`;
        // }
      } else {
        if (bookmark === true) {
          var arrList = $('ul.nav.navbar-nav.bookmark-icons li'),
